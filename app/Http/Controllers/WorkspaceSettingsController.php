...
final class WorkspaceSettingsController extends Controller
{
    ...
    {
        return view('settings.business', ['workspace' => $context->get()]);
    }

    public function update(Request $request, WorkspaceContext $context): RedirectResponse
    {
        $workspace = $context->get();
        $data = $request->validate(['name' => ['required', 'string', 'max:120'], 'legal_name' => ['nullable', 'string', 'max:180'], 'email' => ['nullable', 'email', 'max:255'], 'phone' => ['nullable', 'string', 'max:40'], 'address' => ['nullable', 'string', 'max:1000'], 'tax_id' => ['nullable', 'string', 'max:80'], 'currency' => ['required', 'string', 'size:3'], 'timezone' => ['required', 'timezone:all'], 'locale' => ['required', 'string', 'max:12'], 'quotation_prefix' => ['required', 'alpha_num', 'max:12'], 'default_validity_days' => ['required', 'integer', 'between:1,365'], 'payment_instructions' => ['nullable', 'string', 'max:2000'], 'brand_color' => ['required', 'regex:/^#[0-9A-Fa-f]{6}$/']]);
